...
export default function AiToolsPage() {
  ...
  return (
    <main className="min-h-screen bg-neutral-950 flex flex-col justify-between">
      ...
      <section className="max-w-3xl mx-auto w-full px-4 py-16 animate-fade-in">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          {tools.map((tool) => (
            <div key={tool.title} className="card-premium flex flex-col items-center gap-8 p-8 md:p-12 min-h-[180px] shadow-xl hover:scale-[1.02] hover:shadow-2xl transition-all duration-300">
              ...
              <div className="flex flex-col items-center w-full mt-6 md:mt-0">
                <Link href={isSignedIn ? tool.href : `/sign-in?redirect=${tool.href}`} className="btn-premium text-lg px-8 py-3 w-full text-center">{tool.cta} →</Link>
              </div>
            </div>
          ))}
        </div>
