...
export default function AiProAccessPage() {
  ...
  const parseSections = (text: string) => {
    const extract = (label: string) => {
      const patterns = [
        new RegExp(`\\*\\*\\d+\\.\\s*${label}\\*\\*\\s*([\\s\\S]*?)(?=\\*\\*\\d+\\.|$)`, 'i'),
  ...
  return (
    <main className="max-w-3xl mx-auto px-6 py-14 bg-white">
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow-2xl rounded-3xl p-10 space-y-10 border border-gray-100">
        <section>
          <h2 className="text-gray-600 text-xs font-bold uppercase mb-4 tracking-wider">Required Details</h2>
          <div className="grid gap-6">
            <InputField icon={<Tag />} name="niche" label="Niche" example="e.g. Fitness Coaching, Fashion Retailer" value={formData.niche} onChange={handleChange} />
            <InputField icon={<Megaphone />} name="platform" label="Platform" example="e.g. Instagram, TikTok, LinkedIn" value={formData.platform} onChange={handleChange} />
            <InputField icon={<User />} name="audience" label="Audience" example="e.g. Moms 30-45, Entrepreneurs, Students" value={formData.audience} onChange={handleChange} />
            <InputField icon={<Mic />} name="tone" label="Tone" example="e.g. Funny, Educational, Bold" value={formData.tone} onChange={handleChange} />
            <InputField icon={<TrendingUp />} name="goal" label="Goal" example="e.g. Increase engagement, grow to 10k followers" value={formData.goal} onChange={handleChange} />
            <div>
              <label htmlFor="format" className="block font-medium text-gray-700 mb-1 flex items-center gap-2">
        ...
        <section>
          <h2 className="text-gray-600 text-xs font-bold uppercase mb-4 tracking-wider">Optional Enhancers</h2>
          <div className="grid gap-6">
            <InputField icon={<Target />} name="product" label="Product or Offer" example="e.g. Digital course, Apparel drop" value={formData.product} onChange={handleChange} />
            <InputField icon={<AlertCircle />} name="pain" label="Customer Pain Point" example="e.g. Struggling with consistency, fear of failure" value={formData.pain} onChange={handleChange} />
          </div>
        </section>
...
function InputField({ name, label, value, onChange, example, icon }: {
  name: string;
  label: string;
  value: string;
  ...
  return (
    <div>
      <label htmlFor={name} className="block font-medium text-gray-700 mb-1 flex items-center gap-2">
        {icon && <span className="text-gray-500 w-4 h-4">{icon}</span>}
        {label}
      </label>
      {example && <span className="block text-xs text-gray-400 font-normal mb-1">{example}</span>}
